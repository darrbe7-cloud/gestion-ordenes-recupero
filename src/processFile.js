...
async function processUploadedFile(filePath, originalName) {
  ...
  try {
    ...
    for await (const worksheetReader of workbookReader) {
      for await (const row of worksheetReader) {
        ...
        if (!headers) {
          ...
          ['STATUS_ORDEN', 'ORIGEN', 'STATUS_AB', 'COMUNA', 'TIPO', 'RUT', 'NOMBRE', 'DIRECCION', 'REGION'].forEach(function (col) {
    ...
    var regionesSet = {};
    Object.keys(regionByComuna).forEach(function (c) { regionesSet[regionByComuna[c]] = true; });
    var regionesUnicas = Object.keys(regionesSet).sort();
    ...
    await db.setMetaMany([
      ...
      ['REGIONES_JSON', JSON.stringify(regionesUnicas)]
    ]);
